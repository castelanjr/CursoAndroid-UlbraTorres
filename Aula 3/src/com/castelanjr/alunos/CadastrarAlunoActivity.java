...
public class CadastrarAlunoActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        // Se não for passado nenhum valor, é retornado -1 pelo método
        // getIntExtra()
        // Ou seja: se for -1, é porque não estamos no modo de edição, e sim de
        // criação de alunos
        ...
        if (idAluno == -1) {
            ...
            // Busca o aluno do banco de dados e reenche os campos com as
            // informações dele
    ...
    private void salvar() {
        if (editNome.getText().toString().trim().length() > 0
                && editIdade.getText().toString().trim().length() > 0) {
            ...
            Toast.makeText(CadastrarAlunoActivity.this, "Preencha todos os campos",
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (item.getItemId() == R.id.menu_adicionar) {
            ...
            // Ao selecionar a opção de Descartar, se estiver no modo de criar
            // novo aluno, limpa os campos normalmente usando o método
            // descartar()
            // Caso contrário, se estiver no modo de edição (MODO_EDITAR_ALUNO),
            // exclui o mesmo do banco de dados
            if (modo == MODO_NOVO_ALUNO) {
                descartar();
            } else {
                excluirAluno();
